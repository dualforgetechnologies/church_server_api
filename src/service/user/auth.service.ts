...
import { UserWithRelations } from "@/types/member";
...
export class AuthService extends Service {
  ...
  async refreshToken(dto: RefreshTokenDto): Promise<AppResponse> {
    return this.run(async () => {
      ...
      const refreshToken = await this.jwtService.rotateRefreshToken(
        dto.refreshToken,
        "USER",
      );
  ...
  async requestPasswordReset(dto: ForgotPasswordDto): Promise<AppResponse> {
    return this.run(async () => {
      ...
      if (!user) {
        return this.success({
          message:
            "Reset password instructions sent if the email is registered.",
        });
      }
      if (user?.tenantId) {
        tenant = await this.tenantRepo.findUnique({ id: user.tenantId });
      }
      const expiry = getExpiryDateFromNow(
        AppConfig.jwt.resetLinkExpiresIn as StringValue,
      );
  ...
  async socialLogin(
    ...
    return this.run(async () => {
      ...
      if (user.role === UserRole.GUEST) {
        return this.error(
          "Unauthorized: Must belong to an organization",
          StatusCodes.UNAUTHORIZED,
        );
      }
      await this.userRepo.update(
  ...
  async getProfile(user: User): Promise<AppResponse> {
    return this.run(async () => {
      ...
      const { member,passwordHash,...rest } =existing as UserWithRelations
      const {CommunityMember,...other}=member??{}
