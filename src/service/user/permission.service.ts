...
export class PermissionService extends Service {
  ...
  constructor() {
    ...
    this.userPermissionOverrideRepo = new UserPermissionOverrideRepository(
      this.prisma
    );
  }
  ...
  async assignPermissionsToRole(
    ...
    return this.run(async () => {
      const assigned: Prisma.RolePermissionCreateInput[] =
      ...
      const created = await Promise.all(
        assigned.map((data) => this.rolePermissionRepo.create(data))
      );
  ...
  async removeUserPermissionOverride(
    dto: RemoveUserPermissionOverrideDto,
    actingUser: User
  ): Promise<AppResponse> {
    return this.run(async () => {
  ...
  async getEffectivePermissions(
    query: EffectivePermissionsQueryDto
  ): Promise<AppResponse> {
    return this.run(async () => {
      ...
      const userOverrides =
        await this.userPermissionOverrideRepo.findActiveByUser(query.userId);
